...
{
	Buffer *buffer ;
	buffer = mmap(&global_buffer, sizeof(Buffer), PROT_WRITE | PROT_READ, MAP_SHARED, shm_id, 0) ;
	...
	munmap((void*)buffer, sizeof(Buffer)) ;
...
void child(int shm_id)
{
	...
	void *temp ;
	temp = mmap(&global_buffer, sizeof(Buffer), PROT_READ | PROT_WRITE, MAP_SHARED, shm_id, 0) ;		// hint: modify this line
