...
typedef struct {
	...
	pthread_mutex_t mutex ;		// to ensure mutual exclusion of functions
...
int main(int argc, char *argv[])
{
	...
	for(int i = 0 ; i < 5 ; i++) pthread_cond_signal(&dp.var[i]) ;
	repeat = FALSE;
...
// TO DO: implement this function
{
	...
	for(int i = 0 ; i < 5 ; i++) pthread_cond_init(&dp->var[i], NULL) ;
}
...
// TO DO: implement this function
{
	...
	for(int i = 0 ; i < 5 ; i++) pthread_cond_destroy(&dp->var[i]) ;
}
...
// 	      i.e., running this function from multiple threads simultaneously MUST be prevented.
{
	dp->state[i] = HUNGRY ;
	Test(dp, i) ;
	if(dp->state[i] != EATING) pthread_cond_wait(&dp->var[i], &dp->mutex) ;
}
